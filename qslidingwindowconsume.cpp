 ...
 *   原数据读取功能为阻塞模式，添加此函数可以调定是否为阻塞模式
 * 在网络通讯的模式下，发送缓冲区是否为空，如果为空，且超过心跳包发送时间，则需发送心跳包
 * 因此，需要使用非阻塞模式
 *
 * 将数据从缓冲区dataBuffer中按指定格式读到data_buffer中，读取其中一帧数据
 *    输入:
 *       char *data_buffer:存放一帧数据的缓冲区
 *       locket:  == 1:阻塞模式  ==0:非阻塞模式
 *    输出:
 *       成功:数据长度  失败:-1
 ******************************************************************
*/
int QSlidingWindowConsume::read_data_to_buffer(char *data_buffer,int locked)
{
heads:
